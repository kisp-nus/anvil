...
let verification_run (argv : string array) : unit =
  (* args *)
  let anvil_file_path            = argv.(2) in
  let anvil_generated_sv_path    = argv.(3) in
  let user_sv_file               = argv.(4) in

  AssertName.generate anvil_generated_sv_path user_sv_file;

  let config = make_config anvil_file_path in
  verification_compile stdout config;
